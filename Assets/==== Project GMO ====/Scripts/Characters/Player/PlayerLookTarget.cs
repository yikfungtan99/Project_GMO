...
public class PlayerLookTarget : MonoBehaviour
{
    ...
    public IHealth currentTargetHealth { get; private set; }
    ...
    void Update()
    {
        if (Physics.Raycast(playerAim.position, playerAim.forward, out hit, detectDistance))
        {
            ...
            if(hitObject != hit.collider.gameObject)
            {
                ...
                if (hitObject.GetComponentInParent<IHealth>() != null)
                {
                    currentTargetHealth = hitObject.GetComponentInParent<IHealth>();
                }
